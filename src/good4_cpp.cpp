		...
		for (int j = (1 + i); j < r; ++j) {
			for (int k = 0; k < s; ++k) {
				...
				if (dat_vec[i + (r * k)] == dat_vec[j + (r * k)]) {
					agreement_sum = agreement_sum + (w[k]*(freq_rel[c + (freq_rel_r * k)]*freq_rel[c + (freq_rel_r * k)]));
				}
			}
			if (i == j) {
				good4[j + (i * r)] = 0;
				...
				good4[i + (j * r)] = (1 - (1 / sum_w * agreement_sum));
				good4[j + (i * r)] = good4[i + (j * r)];
			}
			agreement_sum = 0;
		}
	}
